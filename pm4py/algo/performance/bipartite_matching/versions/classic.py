...
# from pm4py.algo.performance.bipartite_matching.util import helper
# from pm4py.algo.performance.bipartite_matching.util import filtering
# import pm4py.algo.performance.bipartite_matching.util.shared_variables as sv
from ..util import helper
from ..util import filtering
...
def apply(log, start, end, classifier="concept:name"):
    ...
    ------------
    result
        {Case ID: (edges selected, measures)}
    """
    ...
    edges_dictio, measures_dictio = update(start, end)

    return edges_dictio, measures_dictio
...
def update(start, end):
    ...
    sv.set_filtered_log(log_filtered)
    log_trimmed = filtering.trim_log(log_filtered)

    edges_dictio = dict()
    measures_dictio = dict()

    for trace in log_trimmed:
        edges, measures = bipartite_matching_measuring(trace)
        edges_dictio[trace.attributes["concept:name"]] = edges
        measures_dictio[trace.attributes["concept:name"]] = measures
...
def bipartite_matching_measuring(trace):
    ...
    Returns
    ------------
    edges selected, measures
        The edge selected and the corresponding measurements
    """
...
def encode_trace_to_graph(trace):
    ...
    for event in trace:
        graph.add_node(event["@@eventindex"], name=event["concept:name"], time=event["time:timestamp"], attri=event[sv.classifier])
